...
namespace Realty_Biz.Repository
{
    public class MockOwnerRepository : IOwnerRepository
    {
        ...
        public async Task<Owner> Create(Owner obj)
        {
            ...
            if (data.Where(x => x.NumTitul?.ToUpper() == obj.NumTitul?.ToUpper()).Count() > 0) throw new Exception("Элемент уже существует в списке.");
        ...
        public async Task<Owner> Update(Owner obj)
        {
            ...
            if (objFromData != null)
            {
                objFromData.Fio = obj.Fio;
                objFromData.StartTitul = obj.StartTitul;
                objFromData.TypeTitul = obj.TypeTitul;
                objFromData.NumTitul = obj.NumTitul;
                objFromData.EndTitul = obj.EndTitul;
                objFromData.HouseId = obj.HouseId;
