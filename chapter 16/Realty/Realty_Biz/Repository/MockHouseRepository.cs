...
namespace Realty_Biz.Repository
{
    public class MockHouseRepository : IHouseRepository
    {
        ...
        public MockHouseRepository()
        {
            data = new List<House>()
            {
                new House() { Id = 1, Lot="111", Address = "Москва, Загорьевская ул., 10", IsExclusive=true, IsMortagege=true, NumOfRoms=2, Squeare=121.7d, Price=8900000.0d, RegionId=1 },
                new House() { Id = 2, Lot="222", Address = "Московская область, Химки, Ленинградское шоссе, 20-й километр", IsExclusive=false, IsMortagege=true, NumOfRoms=5, Squeare=135.0d, Price=7400000.0d, RegionId=2,  }
            };
        }
        ...
        public async Task<House> Create(House obj)
        {
            ...
            if (data.Where(x => x.Lot == obj.Lot).Count() > 0) throw new Exception("Элемент уже существует в списке.");
        ...
        public async Task<House> Update(House obj)
        {
            ...
            if (objFromData != null)
            {
                objFromData.Lot = obj.Lot;
                objFromData.Address = obj.Address;
                objFromData.Notes = obj.Notes;
                objFromData.IsExclusive = obj.IsExclusive;
                objFromData.IsMortagege = obj.IsMortagege;
                objFromData.Squeare = obj.Squeare;
                objFromData.NumOfRoms= obj.NumOfRoms;
                objFromData.Price = obj.Price;
                objFromData.ImageUrl = obj.ImageUrl;
                objFromData.RegionId = obj.RegionId;
